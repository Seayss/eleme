...
class Eleme {
    ...
    static parseCookie(cookie) {
        try {
            ...
            if (cookie) {
                ...
                const snsInfo = JSON.parse(snsInfoStr);
    ...
    constructor(cookie, proxy) {
        this.cookie = Object.assign({}, cookie);
     ...
     */
    async getHongbao(sn, headimgurl = "", nickname = "") {
        const { data } = await this.request.send("post", `/restapi/marketing/promotion/weixin/${this.cookie.openid}`, {
            device_id: "",
            group_sn: sn,
            hardware_id: "",
            method: "phone",
     ...
     */
    async getLuckyNumber(sn, theme_id = "0") {
        const { data: { lucky_number } } = await this.request.send("get", `/restapi/marketing/themes/${theme_id}/group_sns/${sn}`);
        return lucky_number;
    }
    /**
    ...
    async changeMobile() {
        try {
            await this.request.send("post", `/restapi/marketing/hongbao/weixin/${this.cookie.openid}/change`, {
                phone: this.mobile,
                sign: this.cookie.sign
            }, {
                headers: {
     ...
     */
    async loginByMobile(validateCode) {
        const { data: { user_id }, headers } = await this.request.send("post", "/restapi/eus/login/login_by_mobile", {
            mobile: this.mobile,
     ...
     * @returns {Promise<string>}
     */
    async sendMobileCode(mobile, captcha_hash = "", captcha_value = "") {
        const { data: { validate_token } } = await this.request.send("post", "/restapi/eus/login/mobile_send_code", {
            mobile,
            captcha_hash,
            captcha_value
        });
